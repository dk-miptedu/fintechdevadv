...
class TestConfigInit(unittest.TestCase):
    ...
    @patch('os.path.isfile', side_effect=lambda path: path == '.config/__fin_bot_config.yaml')  
    @patch('os.path.exists', side_effect=lambda path: path not in ['app_data', 'db_users', '.config/__fin_bot_config.yaml'])
...
token_tg: 7361445538:AAFu24WyPLujFGbmFHWMBxjyXRWU_l2K6Gg
    ...
    def test_config_loading_and_directory_creation(self, mock_makedirs, mock_open, mock_exists, mock_isfile):
        ...
        mock_open.assert_called_once_with('.config/__in_bot_config.yaml', 'rt')
